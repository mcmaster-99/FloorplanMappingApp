...
SVG.on(document, 'DOMContentLoaded', function () {
    ...
    var drawing = new SVG('svgGrid').size("100%", "100%").panZoom({ zoomMin: 0.5, zoomMax: 2, zoomFactor: 0.1 });
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    if (result[i].rooms.length > 0) {
                        ...
                        for (var j = 0; j < result[i].rooms.length; j++) {
                            ...
                            var room = drawing.rect(result[i].rooms[j].width, result[i].rooms[j].height).attr({
            ...
            for (var _i = 0; _i < currentFloorPlan.length; _i++) {
                ...
                if (currentFloorPlan[_i].rooms.length > 0) {
                    ...
                    for (var j = 0; j < currentFloorPlan[_i].rooms.length; j++) {
                        ...
                        // change created room's id to roomID from database
                        room.node.id = currentFloorPlan[_i].rooms[j].roomID;
                        // store roomID in var
                        var room_ID = room.node.id;
            ...
            */
            for (var i = 0; i < floorPlanSvg.length; i++) {
                ...
                mouseX > roomX - clickMarginError && mouseY < roomY + roomHeight - doorLength / 2 // above bottom roomY
                        ...
                        var door = drawing.line(mouseX, mouseY - doorLength / 2, mouseX, mouseY + doorLength / 2).stroke({ color: '#888888', width: 3 });
                    ...
                    mouseY > roomY - clickMarginError && mouseX < roomX + roomWidth - doorLength / 2) {
                        var door = drawing.line(mouseX - doorLength / 2, mouseY, mouseX + doorLength / 2, mouseY).stroke({ color: '#888888', width: 3 });
